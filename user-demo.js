...
app.post('/login', (req, res) => {
    ...
    if(!body.id || !body.pwd){
        return res.status(404).send("아이디와 비밀번호를 입력하세요")
    }

    if((user == body.id ) || (user.pwd == body.pwd)){
        res.json(`${user.name}님 환영합니다.`)
    } else {
        res.status(404).send("아이디나 비밀번호가 틀렸습니다.")
    }
})
// 회원가입
app.post('/join', (req, res) => {
    ...
    if(!body.id || !body.pwd || !body.name){
        return res.status(404).send("아이디와 비밀번호와 이름을을 입력하세요")
    }
...
app.get('/users/:id', (req, res) => {
    ...
    if(db.has(id)){
        ...
        res.status(404).send("입력하신 아이디는 없습니다.")
    }
})
// 회원탈퇴
